...
func IsCapsuleUser(ctx context.Context, req admission.Request, clt client.Client, userGroups []string, excludedUsers []string) bool {
	...
	// if the user part of ExcludedUsers ignore it
	if isUserExcluded(ctx, req.UserInfo.Username, excludedUsers) {
		return false
	}
	//nolint:nestif
...
// Check if the user is excluded in capsule configurations ExcludedUsers.
func isUserExcluded(_ context.Context, user string, excludedUsers []string) bool {
	for _, item := range excludedUsers {
		if item == user {
			return true
		}
	}

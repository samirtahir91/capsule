...
var _ = Describe("creating a Namespace with Tenant name prefix enforcement at Tenant scope", func() {
	...
	It("should fail using prefix without capsule.clastix.io/tenant label, for a tenant with ForceTenantPrefix true", func() {
		ns := NewNamespace("awesome-namespace")
		NamespaceCreation(ns, t1.Spec.Owners[0], defaultTimeoutInterval).ShouldNot(Succeed())
	})

	It("should succeed and be assigned with prefix and label, for a tenant with ForceTenantPrefix true", func() {
		labels := map[string]string{
			"capsule.clastix.io/tenant": t1.GetName(),
		}
	...
	It("should fail when not using prefix, with tenant label for a tenant with ForceTenantPrefix true and global ForceTenantPrefix true in capsuleConfiguration", func() {
		ModifyCapsuleConfigurationOpts(func(configuration *capsulev1beta2.CapsuleConfiguration) {
	...
	It("should succeed and be assigned with prefix and label, for a tenant with ForceTenantPrefix true and global ForceTenantPrefix true in capsuleConfiguration", func() {
		ModifyCapsuleConfigurationOpts(func(configuration *capsulev1beta2.CapsuleConfiguration) {
			configuration.Spec.ForceTenantPrefix = true
		})
		labels := map[string]string{
	...
	It("should fail using prefix without capsule.clastix.io/tenant label, for a tenant with ForceTenantPrefix true and global ForceTenantPrefix true in capsuleConfiguration", func() {
		ModifyCapsuleConfigurationOpts(func(configuration *capsulev1beta2.CapsuleConfiguration) {
			configuration.Spec.ForceTenantPrefix = true
		})
		ns := NewNamespace("awesome-namespace")
